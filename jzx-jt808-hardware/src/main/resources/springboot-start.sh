...
cd `dirname $0`
# 应用名（boot jar包名）需要修改地方
APP_NAME.jzx.jt808-hardware
...
JVM_OPTIONS="-server -Xms128m -Xmx256m -XX:MetaspaceSize=128m $GC_OPTS $GC_LOG_OPTS $DUMP_OPTS $OTHER_OPTS"
...
start(){
  ...
  if [ $? -eq "0" ]; then 
    echo "--- ${JAR_NAME} is already running PID=${pid} ---" 
      ...
      echo "JVM_OPTIONS : "
      echo "$JVM_OPTIONS -Dspring.profiles.active=$PROFILE"
